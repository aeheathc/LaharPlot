...
int main(int ac, char *av[]) {
	...
	char * sdemText = (char*) "Read topography from file with given SDEM-file name.  Can't "
					  "be used with simple_name.  (File type = <arg>-sdem.tsv)\n";
	...
	char * verbText = "Display detailed information.  Due to multi-threading, it is highly "
					  "recommended that you only use verbose mode when there is only one volume.\n";

	char * statText = "Display a status output every <arg> seconds while calculating the "
...
int createIZM(IZMData data, double volume, int ID, MapperStatus * status) {
	...
	while(1) {
		...
		if ( newRX < 0 || newRX > (xCells - 1) || newRY < 0 || newRY > (yCells -1) ) {
			...
			string end = "Stream cell has traveled off the map @ [" + rXStr + "][" + rYStr + "].";
			status->setEndCondion(ID, end);
			break;
		}
		if ( (newRX == endX) && (newRY == endY) ) {
			...
			string end = "Stream cell has reached the ending cell @ [" + rXStr + "][" + rYStr + "].";
			status->setEndCondion(ID, end);
			break;
		}
	}
	...
	while(1) {
		...
		// Both left and right cells could be equal or lower than fill level
			...
			if (rightLevel == fillLevel) {
				...
				if (v)
					cout << "			The right level is equal to the fill level so extending cross section." << endl;
				...
				if (v)
					cout << "				Tier area is from fall is " << tierArea << " :: FL = " << fillLevel
			             << " || RL = " << rightLevel << " || LL = " << leftLevel
			             << " || CSW = " << crossWidth << " || CellW = " << cWidth << endl;
			...
			if (leftLevel == fillLevel) {
				...
				if (v)
					cout << "			The left level is equal to the fill level so extending cross section." << endl;
